/* ============================================================
standard-deck.js v2.0 — Core Rendering Engine
...
var SAFE = { x: 0.42, y: 0.56, x2: 12.92, y2: 7.00 };
...
function validateElement(el, slide, slideIndex) {
 ...
 if (typeof el.x === 'number' && el.x < SAFE.x) el.x = SAFE.x;
 if (typeof el.y === 'number' && el.y < SAFE.y) el.y = SAFE.y;
 if (el.type === 't' && el._parentShape) {
   var maxW = el._parentShape.w * 0.80;
   if (el.w > maxW) {
...
function renderText(el, isDark) {
 ...
 div.style.cssText = 'position:absolute;box-sizing:border-box;overflow:hidden;word-wrap:break-word;';
